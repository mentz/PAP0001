...
int main (int argc, char **argv) {
	...
	*/
	...
	Color linha, preto = {0, 0, 0, 1};
	linha.r = linha.g = linha.b = 255;
	int xinterval = vinf.xres * 15 / 360;
	int yinterval = vinf.yres * 15 / 180;
		...
		for (uint y = 0; y < vinf.yres; y++) {
			for (uint x = 0; x < vinf.xres; x++) {
				...
				if ((x % xinterval) == 0 || (y % yinterval) == 0)
				{
					...
					*/
					s.buffer[pix_offset + s.red]   = linha.r;
					s.buffer[pix_offset + s.green] = linha.g;
					s.buffer[pix_offset + s.blue]  = linha.b;
				}
				else
				{
					s.buffer[pix_offset + s.red]   = preto.r;
					s.buffer[pix_offset + s.green] = preto.g;
					s.buffer[pix_offset + s.blue]  = preto.b;
				}
				pixel_id++;
			}
		}
